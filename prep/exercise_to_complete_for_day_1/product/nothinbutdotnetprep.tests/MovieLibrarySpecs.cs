...
namespace nothinbutdotnetprep.tests
{
    public class MovieLibrarySpecs
    {
        ...
        public class when_searching_for_movies : searching_and_sorting_concerns_for_movie_library
        {
            ...
            {
                var results = sut.all_movies().that_match(Where<Movie>.has_a(x => x.production_studio).equal_to(ProductionStudio.Pixar));
                results.should_only_contain(cars, a_bugs_life);
            };

            it should_be_able_to_find_all_movies_published_by_pixar_or_disney = () =>
            {
                var results = sut.all_movies().that_match(Where<Movie>.has_a(movie => movie.production_studio).equal_to_any(ProductionStudio.Pixar, ProductionStudio.Disney));
            ...
            {
                var results = sut.all_movies().that_match(Where<Movie>.has_an(x => x.date_published.Year).between(1982, 2003));
        ...
        public class when_sorting_movies : searching_and_sorting_concerns_for_movie_library
        {
            ...
            {
                var results = sut.all_movies().sort(Sort<Movie>.by_desc(x => x.title).build());
            ...
            it should_be_able_to_sort_all_movies_by_title_ascending = () =>
            {
                ...
                var results = sut.all_movies().sort(Sort<Movie>.by(x => x.title).build());
            ...
            {
                var results = sut.all_movies().sort(Sort<Movie>.by_desc(x => x.date_published).build());
            ...
            it should_be_able_to_sort_all_movies_by_studio_rating_and_year_published = () =>
            {
                ...
                //Disney
                var results =
                    sut.all_movies().sort(Sort<Movie>.by_desc(new StudioRankComparer()).then_by(x => x.date_published.Year).build());
